...
// Endpoint for Tag products
const API_URL = "https://craftology-backend.onrender.com/api/tags";
...
// --- INTERFACES ---
export type TagItem = CoinItem;

interface TagProps {
  onBack?: () => void;
}

export default function Tag({ onBack }: TagProps) {
  // --- STATE ---
  ...
  const [selectedProduct, setSelectedProduct] = useState<TagItem | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Checks if jsonData is an array, or looks inside .data or .tags
        const itemsArray = Array.isArray(jsonData)
          ? jsonData
          : jsonData.data || jsonData.tags || [];
        ...
        console.error(err);
        setError("Failed to load Tag collection.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const openModal = (product: TagItem) => {
          ...
          <div ref={marqueeTrackRef} className="flex whitespace-nowrap w-max">
            ...
            <div className="flex items-center gap-8 md:gap-16 px-4 md:px-8 flex-shrink-0">
              {Array.from({ length: 4 }).map((_, i) => (
                ...
                >
                  <span>The Perfect Finish</span> •{" "}
                  <span>Chat on WhatsApp to Order</span> •{" "}
                  <span>Handcrafted Tags</span> •
                </span>
              ))}
            </div>
            {/* Set 2 */}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 py-12 min-h-full flex flex-col pt-36 md:pt-24 relative z-10">
        ...
        <div className="text-center mb-8 md:mb-16 space-y-3 md:space-y-4">
          ...
          <p className="opacity-80 max-w-lg mx-auto text-sm md:text-lg leading-relaxed font-light text-[#371E10]">
            Small details that speak volumes. Beautifully crafted tags to add a
            personal touch to your gifts and hampers.
          </p>
        </div>
