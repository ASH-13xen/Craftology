...
// Endpoint for Toran products
const API_URL = "https://craftology-backend.onrender.com/api/torans";
...
// --- INTERFACES ---
export type ToranItem = CoinItem;

interface ToranProps {
  onBack?: () => void;
}

export default function Toran({ onBack }: ToranProps) {
  // --- STATE ---
  const [data, setData] = useState<ToranItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [currentPage, setCurrentPage] = useState(1);
  const [selectedProduct, setSelectedProduct] = useState<ToranItem | null>(
    null,
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Checks if jsonData is an array, or looks inside .data or .torans
        const itemsArray = Array.isArray(jsonData)
          ? jsonData
          : jsonData.data || jsonData.torans || [];
        ...
        console.error(err);
        setError("Failed to load Toran collection.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const openModal = (product: ToranItem) => {
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 py-12 min-h-full flex flex-col pt-36 md:pt-24 relative z-10">
        ...
        <div className="text-center mb-8 md:mb-16 space-y-3 md:space-y-4">
          ...
          <p className="opacity-80 max-w-lg mx-auto text-sm md:text-lg leading-relaxed font-light text-[#371E10]">
            Welcoming entryways with traditional charm. Handcrafted torans
            designed to bring prosperity and beauty to your home.
          </p>
        </div>
